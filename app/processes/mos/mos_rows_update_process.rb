...
class Mos::MosRowsUpdateProcess
  class << self
    ...
    def call_via_api(dataset_id)
      result = MosAdapter.get_datasets_count(dataset_id: dataset_id)
      return unless result[:success]
      ...
      skip = 0
      loop do
        result = MosAdapter.get_rows(dataset_id: dataset_id, top: top, skip: skip)
